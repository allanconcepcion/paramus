...
// Faust.js' media URL rewriting can incorrectly double up the protocol
// (e.g. "https:https://example.com/image.jpg") when processing srcset
// strings that contain multiple comma-separated URLs. This strips any
// such duplicated protocol prefix so the browser can load the image.
const fixDoubleProtocol = (value) => {
        if (!value || typeof value !== 'string') return value
        return value.replace(/https?:(https?:\/\/)/g, '$1')
}
...
const WPImage = ({
        ...
        const renderImage = () => {
            ...
            >
                                    <Image
                    src={fixDoubleProtocol(responsiveImage.sourceUrl && typeof(responsiveImage.sourceUrl) === 'function' ? responsiveImage.sourceUrl() : responsiveImage.sourceUrl)}
                    srcSet={fixDoubleProtocol(responsiveImage.srcSet && typeof(responsiveImage.srcSet) === 'function' ? responsiveImage.srcSet() : responsiveImage.srcSet)}
